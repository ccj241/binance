...
func processUserOrders(cfg *config.Config, userID uint, orders []models.Order) {
	...
	// 验证解密后的API Key格式
	if apiKey == "" {
		log.Printf("用户 %d 解密后的API Key为空", userID)
		return
	}

	// 币安API Key应该是64个字符
	if len(apiKey) != 64 {
		log.Printf("用户 %d API Key格式错误，长度=%d，期望=64", userID, len(apiKey))
		return
	}
	...
	// 验证解密后的Secret Key格式
	if secretKey == "" {
		log.Printf("用户 %d 解密后的Secret Key为空", userID)
		return
	}

	// 币安Secret Key也应该是64个字符
	if len(secretKey) != 64 {
		log.Printf("用户 %d Secret Key格式错误，长度=%d，期望=64", userID, len(secretKey))
		return
	}

	// 添加调试日志（注意：生产环境应该移除或脱敏）
	log.Printf("用户 %d API密钥解密成功，API Key前6位: %s..., Secret Key前6位: %s...",
		userID, apiKey[:6], secretKey[:6])
